...
namespace SS
{
	...
	bool Settings::LoadFrom(const std::string& a_path)
	{
		...
		Get(table, "player", "power", sensePower);
		Get(table, "player", "shout", senseShout);
		Get(table, "player", "readyFlash", readyFlash);
		Get(table, "player", "readyFlashColour", readyFlashColour);
		Get(table, "player", "glyphGlow", glyphGlow);
		Get(table, "player", "glowShader", glowShader);
	...
	bool Settings::SaveAs(const std::string& a_path) const
	{
		...
		file << "; The chosen power, full when ready and hollow while it waits.\n";
		file << "power = " << boolean(sensePower) << "\n";
		file << "; The Thu'um: a rune filling as the voice recovers; a plain ring before\n";
		file << "; you can shout at all.\n";
		file << "shout = " << boolean(senseShout) << "\n";
		file << "; A moment's light over your bars when a cooldown comes back.\n";
		file << "readyFlash = " << boolean(readyFlash) << "\n";
		file << "readyFlashColour = " << ColourText(readyFlashColour) << "\n";
		file << "; A halo behind a weapon or spell glyph in its own colour.\n";
		file << "glyphGlow = " << boolean(glyphGlow) << "\n";
		file << "; The glows as real light from a shader pass, added over the frame,\n";
		file << "; instead of shapes drawn in the HUD. Fine under Community Shaders.\n";
		file << "glowShader = " << boolean(glowShader) << "\n";
